h37969
s 00017/00025/01832
d D 5.2 05/04/17 04:26:59 msokolov 2 1
c always use syslog
e
s 01857/00000/00000
...
static char rcsid[] = "$Header: readfile.c,v 1.8 88/11/15 23:50:18 ddp Exp $";
#endif
...
#ifdef SYSLOG
E 2
	syslog(LOG_INFO, "bootptab mtime is %s\n",
	       timestr);
D 2
#endif
E 2
...
E 2
I 2
		syslog(LOG_ERR, "duplicate hardware address: %s\n",
E 2
...
#ifdef SYSLOG
E 2
    syslog(LOG_INFO, "read %d entries from \"%s\"", nhosts, bootptab);
D 2
#endif
E 2
...
E 2
I 2
		syslog(LOG_ERR, "syntax error in entry for host \"%s\"\n",
E 2
				host->hostname->string);
		return -1;
...
E 2
I 2
		syslog(LOG_ERR, "unknown symbol in entry for host \"%s\"\n",
E 2
				host->hostname->string);
		return -1;
	    case E_BAD_IPADDR:
...
E 2
I 2
		syslog(LOG_ERR, "bad IP address for host \"%s\"\n",
E 2
				host->hostname->string);
		return -1;
	    case E_BAD_HADDR:
...
E 2
I 2
		syslog(LOG_ERR, "bad hardware address for host \"%s\"\n",
E 2
				host->hostname->string);
		return -1;
	    case E_BAD_SMASK:
...
E 2
I 2
		syslog(LOG_ERR, "bad subnet mask for host \"%s\"\n",
E 2
				host->hostname->string);
		return -1;
	    case E_BAD_TIMEOFF:
...
E 2
I 2
		syslog(LOG_ERR, "bad time offset for host \"%s\"\n",
E 2
				host->hostname->string);
		return -1;
...
E 2
I 2
		syslog(LOG_ERR, "bad vendor magic cookie for host \"%s\"\n",
E 2
				host->hostname->string);
		return -1;
	    case E_END_OF_ENTRY:
...
byte tagvalue;
{
    ...
    *str++ = tagvalue;	    /* Store tag value */
    ...
    if ((*src)[0] == '"') {				/* ASCII data */
	newlength = MAXBUFLEN - 2;	/* Set maximum allowed length */
	(void) get_string(src, str, &newlength);
    } else {						/* Numeric data */
	newlength = 0;
	while (newlength < sizeof(tmpbuf)) {
	...
	oldlength -= 1;				/* Overwrite old TAG_END */
    }
    bcopy(tmpbuf, bdata->data + oldlength, newlength + 2);
    (bdata->data)[oldlength + newlength + 2] = TAG_END;	/* Mark end of data */
...
E 2
I 2
	syslog(LOG_ERR, "can't find tc=\"%s\"\n", tstring);
E 2
    } else {
	/*
 ...
 * message is printed to stderr (and syslog if appropriate) and the program
 * aborts with a status of 1.
 */
...
E 2
I 2
	syslog(LOG_ERR, "malloc() failure -- exiting\n");
E 2
	exit(1);	
    }
