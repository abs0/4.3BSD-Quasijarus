h09486
s 00027/00002/00926
d D 6.4 03/11/18 22:53:14 msokolov 4 3
c document how to load standalone programs from floppy on 8200
e
s 00084/00054/00844
d D 6.3 03/11/18 22:18:05 msokolov 3 2
c update for 4.3BSD-Quasijarus
e
s 00000/00000/00898
d D 6.2 03/11/16 16:24:47 msokolov 2 1
c bump it up to 6.2
e
s 00898/00000/00000
d D 6.1 03/11/16 16:23:06 msokolov 1 0
...
E 3
I 3
the system on an 11/750, no console cassette is usually required.
E 3
.PP
...
I 3
These instructions were originally written for large VAXen
that use console media.
MicroVAXen have no console media, if you are bootstrapping a MicroVAX,
see Appendix B.
.PP
E 3
...
E 3
I 3
off; on an 8600 or 750 set the power-on action to halt.
(In normal operation a 780 or 730 will have the reboot
E 3
switch on and an 8600 or 750 will have the power-on action set
to reboot/restart.)
I 3
On an 8200 the keyswitches should be set to Enable/Update for bootstrapping
and maintenance (Secure/Autostart in normal operation);
on a MicroVAX II the switch on the rear I/O panel should be
turned up to enable halts (halts are disabled in normal operation).
E 3
.PP
...
The
.I format
program is for hp/up SMD disks only.
MSCP disks usually do not require formatting.
If you need to format an MSCP disk, you will need to use formatting utilities
provided by the drive or controller vendor,
.I format
will not help you.
.PP
E 3
...
E 3
I 3
If you have an 8600 and no RL02 pack with UNIX standalone programs
start the bootstrap monitor using the
E 3
...
E 3
I 3
load programs off the distributed console medium,
E 3
...
E 3
I 3
\fIformat\fP program was loaded; for example, if not using console media,
E 3
...
E 3
I 3
\fBCopy completed: 308 records copied\fP
E 3
\fBFrom:\fP
.TE
.DE
...
(copy mini root file system)
I 4
\fB>>>\fP\|\fIB/R5:800 CSA1\fP	(or CSA2 if using the 2nd diskette slot)
\fBBOOT58>\fP\|\fILOAD COPY\fP
\fBBOOT58>\fP\|\fISTART 2\fP
\fBFrom:\fP \fIyy(y,1)\fP	(unit \fIy\fP, second tape file)
\fBTo:\fP \fIxx(x,1)\fP	(mini root is on drive \fIx\fP; second partition)
\fBCopy completed: 308 records copied\fP
\fBFrom:\fP
...
E 3
I 3
instructions in chapter 4.  For 11/750's, bootstrapping is normally performed by
E 3
...
I 3
root file system.
If you do not have a boot prom for your system disk, you may also create
a TU58 cassette with BOOT58 and
.UX
.I boot
program and boot script and use it as your boot path.
If you want to pursue this approach, contact Quasijarus Consortium
for assistance.
VAX 8200 has a very similar boot architecture to 11/750, except that the
bootable disks supported by the ROM are VAXBI and UNIBUS MSCP disks, i.e.,
KDB50 and UDA50, and the console medium for BOOT58 is not TU58 but RX50.
MicroVAXen have no console media.  They instead have a special version of VMB
in the ROM and may only be booted via the bootblock on the disk.
.PP
The
E 3
.I disklabel
...
E 3
I 3
program installs the 750/8200 and MicroVAX boot code along with the pack label.
E 3
.\" XXX needs thought:
...
.DE
I 3
All MicroVAXen \fBmust\fP use the
.I rdboot
primary bootblock regardless of the disk type.
It is the only bootblock for MicroVAXen, all other bootblocks are for 750
and 8200.
If you have a MicroVAX and your disk is something other than RDxx, you must
explicitly specify the
.I rdboot
primary bootblock on the
.I disklabel
command line.
If you are booting from UDA50 or some other UNIBUS MSCP disk on an 8200, the
.I raboot
primary bootblock may not correctly communicate the UNIBUS adapter number to
the secondary bootblock.  It computes the correct uba number for 750, but this
computation probably won't be correct on an 8200.  You will need to either
patch the bootblock or use a BOOT58-based boot path.
.PP
E 3
...
E 3
I 3
UNIBUS CDC 9766	\fB#\fI disk=up; name=up0g; type=9766\fR
E 3
...
E 3
I 3
MASSBUS CDC 9766	\fB#\fI disk=hp; name=hp0g; type=9766\fR
MASSBUS AMPEX 300M	\fB#\fI disk=hp; name=hp0g; type=9300\fR
MASSBUS AMPEX 330M	\fB#\fI disk=hp; name=hp0g; type=capricorn\fR
MASSBUS FUJITSU 330M	\fB#\fI disk=hp; name=hp0g; type=capricorn\fR
MASSBUS FUJITSU 404M	\fB#\fI disk=hp; name=hp0h; type=eagle\fR
E 3
.TE
.DE
...
I 3
Boot blocks are also required on MicroVAXen.
E 3
...
E 3
I 3
\fB#\fI date \fRyyyymmddhhmm	(set date, see \fIdate\fP\|(1))
E 3
\&....
...
I 3
In 1600bpi and TK50 distribution formats the sources are compressed with
\fIcompress\fR\|(1) in order to fit on the tape.
If you are extracting a compressed distribution, modify the following
...
E 3
I 3
There exists additional software that used to be distributed with the system.
Since it is static and does not change with ongoing Quasijarus development, it
has been separated from the main \*(4B distribution.
If you want to install the 4.3BSD-* software supplement, you can obtain and
